...
inline CShellMalloc* _TheShellMallocInst()
{
    static CShellMalloc _theInst;
    _theInst.Init();
    return &_theInst;
}
//////////////////////////////////////////////////////////////////////////////
...
public:
   ...
   BOOL GetPath(std::wstring &sPath)
   {
   ...
   void Concatenate(LPCITEMIDLIST pidl2)
   {
      ...
      LPITEMIDLIST pidlNew = (LPITEMIDLIST) _TheShellMallocInst()->Alloc(cb1 + cb2);
      if( pidlNew != NULL ) {
   ...
   static void PidlDelete(LPITEMIDLIST pidlSource)
   {
      ...
      _TheShellMallocInst()->Free( pidlSource );
   }
   ...
   { 
      ...
      LPITEMIDLIST pidlTarget = (LPITEMIDLIST) _TheShellMallocInst()->Alloc(cbSource); 
...
public:
   ...
   CPidlList(LPITEMIDLIST pidl) : m_pidls(NULL), m_nCount(0)
   {
      ...
      LPITEMIDLIST* pidls = (LPITEMIDLIST*) _TheShellMallocInst()->Alloc(nCount * sizeof(LPITEMIDLIST));   
   ...
   HRESULT Copy(LPCITEMIDLIST* pidlSource, UINT nCount)
   {
      ...
      m_pidls = (LPITEMIDLIST*) _TheShellMallocInst()->Alloc(nCount * sizeof(LPITEMIDLIST));
   ...
   void Delete()
   {
      ...
      for( UINT i = 0; i < m_nCount; i++ ) _TheShellMallocInst()->Free((LPVOID)m_pidls[i]);
      _TheShellMallocInst()->Free(m_pidls);
      m_pidls = NULL;
      m_nCount = 0;
   }
   ...
   HRESULT ReadFromStream(IStream* pStream)
   {
      ...
      LPITEMIDLIST* pList = (LPITEMIDLIST*) _TheShellMallocInst()->Alloc(nCount * sizeof(LPITEMIDLIST));   
   ...
   HRESULT Filter(IShellFolder* pFolder, DWORD dwAttribMask)
   {
      ...
      LPITEMIDLIST* pidls = (LPITEMIDLIST*) _TheShellMallocInst()->Alloc(m_nCount * sizeof(LPITEMIDLIST));
...
public:
   ...
   STDMETHOD(UIActivate)(UINT uState)
   {
      ...
      if( uState != SVUIA_DEACTIVATE ) {
         ...
         TCHAR szName[128] = { 0 };
         ::LoadString(QUIGetApp()->GetResourceInstance(), IDS_DESCRIPTION, szName, (sizeof(szName) / sizeof(TCHAR)) - 1);
      ...
      HWND* phWnd)
   {
      ...
      *phWnd = NULL;
      ...
      ::LoadString(QUIGetApp()->GetResourceInstance(), IDS_CLASSNAME, szClassName, (sizeof(szClassName) / sizeof(TCHAR)) - 1);
      // If our window class has not been registered, then do so now...
      WNDCLASS wc = { 0 };
      if( ::GetClassInfo(QUIGetApp()->GetModuleInstance(), szClassName, &wc) == FALSE ) {
         wc.style          = 0;
         wc.lpfnWndProc    = (WNDPROC)WndProc;
         wc.cbClsExtra     = 0;
         wc.cbWndExtra     = 0;
         wc.hInstance      = QUIGetApp()->GetModuleInstance();
         wc.hIcon          = NULL;
...
#define SHCNF_ACCEPT_NON_INTERRUPTS 0x0002 
   ...
   void _UpdateShellSettings(void)
   {
      ...
      if( QUIGetApp()->m_WinVer.dwMajorVersion == 6 )
      {
...
public:
   ...
   STDMETHOD(AddPages)(LPFNADDPROPSHEETPAGE pfnAddPage, LPARAM lParam)
   {
      ...
      ::LoadString(QUIGetApp()->GetResourceInstance(), T::IDS_TABCAPTION, m_szCaption, (sizeof(m_szCaption) / sizeof(TCHAR)) - 1);

      PROPSHEETPAGE psp = { 0 };
      psp.dwSize        = sizeof(psp);
      psp.dwFlags       = PSP_USETITLE | PSP_USECALLBACK;
      psp.hInstance     = QUIGetApp()->GetResourceInstance();
...
public:
   ...
   STDMETHOD(Extract)(LPCTSTR pszFile, UINT nIconIndex, HICON* phiconLarge, HICON* phiconSmall, UINT nIconSize)
   {
      ...
      if( pData != NULL ) {
         ...
         case 0:
            {
               if( phiconLarge != NULL ) *phiconLarge = ImageList_GetIcon(QUIGetApp()->m_ImageLists.m_hImageListLarge, 0, ILD_TRANSPARENT);
               if( phiconSmall != NULL ) *phiconSmall = ImageList_GetIcon(QUIGetApp()->m_ImageLists.m_hImageListSmall, 0, ILD_TRANSPARENT);
            }
            return S_OK;
         case 1:
            {
               if( phiconLarge != NULL ) *phiconLarge = ImageList_GetIcon(QUIGetApp()->m_ImageLists.m_hImageListLarge, 1, ILD_TRANSPARENT);
               if( phiconSmall != NULL ) *phiconSmall = ImageList_GetIcon(QUIGetApp()->m_ImageLists.m_hImageListSmall, 1, ILD_TRANSPARENT);
            }
            return S_OK;
         case 2:
            {
      ...
      while( pMap->pfn != NULL ) {  
        if( pMap->id == 0 ) {
          ::InsertMenu(hMenu, iIndexMenu++, MF_SEPARATOR | MF_STRING | MF_BYPOSITION, 0, _T(""));
           ...
           TCHAR szText[128] = { 0 };
          ::LoadString(QUIGetApp()->GetResourceInstance(), pMap->id, szText, (sizeof(szText) / sizeof(TCHAR)) - 1);
      ...
      case GCS_HELPTEXTW:
         {
            ...
            while( pMap->pfn != NULL ) {
               if( i == idCmd ) {
                  ...
                  if( uFlags == GCS_HELPTEXTA ) ::LoadStringA(QUIGetApp()->GetResourceInstance(), pMap->desc, pszName, cchMax);                 
                  else ::LoadStringW(QUIGetApp()->GetResourceInstance(), pMap->desc, (LPWSTR) pszName, cchMax);
                  return S_OK;
               }
               pMap++;
               i++;

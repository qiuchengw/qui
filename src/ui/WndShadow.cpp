...
const TCHAR *STR_SHADOWWND_CLASS_NAME = L"ShadowWnd_Modified_QCW";
...
bool CWndShadow::Initialize(HINSTANCE hInstance)
{
	...
	wcex.lpszClassName	= STR_SHADOWWND_CLASS_NAME;
	wcex.hIconSm		= NULL;
...
bool CWndShadow::Create(HWND hParentWnd)
{
	...
	m_hWnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT, 
		STR_SHADOWWND_CLASS_NAME, NULL,WS_POPUP,
		0, 0, 0, 0, hParentWnd, NULL, s_hInstance, NULL);
	if (NULL == m_hWnd)
		return false;
...
LRESULT CWndShadow::ParentProc(HWND hWndParent, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (!IsWindow(m_hWnd) || !IsWindow(hWndParent))
	{
		return 0;
	}
	switch(uMsg)
	...
	case WM_SIZE:
		if(m_Status & SS_ENABLED)
		{
			...
			else
			{
				...
				if(WS_VISIBLE & GetWindowLong(hWndParent, GWL_STYLE))	
				{
...
bool CWndShadow::SetSize(int NewSize)
{
	...
	if((SS_VISABLE & m_Status) && IsValid())
...
void CWndShadow::Show(HWND hParentWnd,BOOL bBeingShow/*=false*/)
{
	...
	if(m_Status & SS_ENABLED)	// Enabled
	{
		...
		if(bBeingShow || (WS_VISIBLE & lParentStyle))	// Parent visible
		{
			m_Status |= SS_PARENTVISIBLE;

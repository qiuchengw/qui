...
struct tabs: public behavior
{
    ...
    { 
      ...
      if (tabs_el.children_count() == 0) // 空的。咱来为它生成代码
	  {
		  dom::element strip = dom::element::create("div");
		  tabs_el.append(strip);
		  strip.set_attribute("class",L"strip");
		  strip.set_state(STATE_CURRENT,0,true); // :current - make tab strip as current element inside focusable.
		  return;
	  }

	  // 
	  dom::element tabitem_el = tabs_el.find_first(":root>.strip>[panel][selected]"); // initialy selected

	  if (!tabitem_el.is_valid())
		return;

      const wchar_t* pname = tabitem_el.get_attribute("panel");
      ...
      dom::element tab_strip_el = tabitem_el.parent();
      tab_strip_el.set_state(STATE_CURRENT,0,true); // :current - make tab strip as current element inside focusable.
      tabitem_el.set_state(STATE_CURRENT,0,true); // :current - current tab is, well, current.

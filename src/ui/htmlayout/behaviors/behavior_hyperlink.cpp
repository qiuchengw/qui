...
/*
BEHAVIOR: linkurl
    goal: Implementation of linkurl notifications.
	跟hyperlink一样，但是这个不会自动加载web页面，而是调用系统函数
	shellexecute 打开浏览器，转到网页
	behavior:linkurl
		href="http://...."
SAMPLE:
   TBD
...
struct linkurl: public behavior
{

    // ctor
    linkurl(): behavior(HANDLE_MOUSE | HANDLE_FOCUS | HANDLE_KEY, 
      "linkurl") {}
    ...
    { 
      if( event_type == KEY_UP )
      {
        if(code != ' ' && code != VK_RETURN)
          return FALSE;
	    notify(he); // send standard notifiaction too. 
        return TRUE;
      }
      return FALSE; 
    }

    void notify(dom::element el)
    {
        const wchar_t *pHREF = el.get_attribute("href");
        if(pHREF)
        {
			ShellExecute(el.get_element_hwnd(true),
				L"open",pHREF,NULL,NULL,SW_SHOW);
        }
    }
};

// instantiating and attaching it to the global list
linkurl linkurl_instance;

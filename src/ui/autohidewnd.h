 ...
 *	窗口被隐藏/显示
 *  wparam      -   reserve
 *  lparam      -   TRUE / 隐藏，FALSE 显示
 */
DECLARE_USER_MESSAGE(MSG_AUTOHIDEWND_WNDVISIBLECHGED);
...
private:
    typedef struct AHW_DATA 
    {
        AHW_DATA(HWND h)
        {
            ...
            nWndOldExStyle = GetWindowLong(hWnd, GWL_EXSTYLE);
        }
        ...
        UINT    nWndOldExStyle;    // 在隐藏的时候，为其设置WS_EX_TOPMOST风格，显示的时候再恢复到这个风格
        BOOL    bHided;     // 是否已经隐藏了
    }*LPAHW_DATA;

...
#else
	return (m_file != NULL && m_file->m_hFile != QFile::hFileNull);
#endif
...
#else
	m_file = new QFile;
	m_file->Open(filename, QFile::modeCreate | QFile::modeWrite);
...
CTextFileWrite::CTextFileWrite(QFile* file, TEXTENCODING encoding)
{
	ATLASSERT(file);
...
CTextFileRead::CTextFileRead(const FILENAMECHAR* filename)
{
	...
	m_file = new QFile;
	//In windows, do this
	m_file->Open(filename, QFile::modeRead | QFile::shareDenyWrite );
	m_closeAndDeleteFile = true;
#endif
...
#if PEK_TX_TECHLEVEL==2
CTextFileRead::CTextFileRead(QFile* file)
{
	ATLASSERT(file);
...
bool CTextFileRead::ReadLine(CStdString& line)
{
#ifndef _UNICODE
	string temp;
#else
...
bool CTextFileRead::Read(CStdString& all, const CStdString newline)
{
#ifndef _UNICODE

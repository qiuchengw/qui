...
// Stream Helpers
CTextFileWrite& operator<<(CTextFileWrite& output, CIniFileW& obj)
{
    obj.Save( output );
    return output;
}

CTextFileRead& operator>>(CTextFileRead& input, CIniFileW& obj)
{
    obj.Load( input );
    return input;
}

CTextFileRead& operator>>(CTextFileRead& input, CIniMergeW merger)
{
    return merger(input);
}
...
void CIniFileW::Save( CTextFileWrite& output )
{
...
bool CIniFileW::Save( const std::wstring& fileName )
{
	...
	CTextFileWrite output(fileName.c_str(),CTextFileWrite::UTF_8);
...
void CIniFileW::Load( CTextFileRead& input , bool bMerge )
{
	if( !bMerge )
		RemoveAllSections();
...
bool CIniFileW::Load(const wstring& fileName , bool bMerge )
{
	CTextFileRead input(fileName.c_str());
	if (!input.IsOpen())
		return FALSE;
	Load( input , bMerge );

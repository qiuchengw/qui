...
public:
    ...
    public:
        ...
        pre_order_iterator(const children_iterator&);
    ...
    public:
        ...
        post_order_iterator(const children_iterator&);
    ...
    public:
        ...
        fixed_depth_iterator(const children_iterator&);
    ...
    public:
        ...
        children_iterator(const children_iterator&);
        children_iterator(const iterator_base&);
    ...
    public:
        ...
        leaf_iterator(const children_iterator&);
    ...
    inline pre_order_iterator begin_pre(const iterator_base& pos)const
    {
        return pre_order_iterator(pos.node);
    }
    /// Return iterator to the end of the tree.
    inline pre_order_iterator   end() const;
    inline pre_order_iterator   end_pre(const iterator_base& pos) const
    {
        // 2013/7/25 qiuchengw 
        // 修复：pos无下一个兄弟节点，遍历将会越过pos节点的子树范围
        assert(pos.node!=0);
        pre_order_iterator itr = pos;
        while(itr.node->next_sibling == 0) 
        {
        ...
        itr.node = itr.node->next_sibling;
        return itr;
    }
    ...
    children_iterator     begin_child(const iterator_base&) const;
    /// Return sibling end iterator for children of given node.
    children_iterator     end_child(const iterator_base&) const;
    ...
    template<typename iter> iter append_children(iter position, children_iterator from, children_iterator to);
    template<typename iter> iter prepend_children(iter position, children_iterator from, children_iterator to);
    ...
    children_iterator insert(children_iterator position, const T& x);
    ...
    children_iterator replace(children_iterator orig_begin, children_iterator orig_end, 
        children_iterator new_begin,  children_iterator new_end); 

    /// Move all children of node at 'position' to be siblings, returns position.
    template<typename iter> iter flatten(iter position);
    /// Move nodes in range to be children of 'position'.
    template<typename iter> iter reparent(iter position, children_iterator begin, children_iterator end);
    ...
    children_iterator move_before(children_iterator target, children_iterator source);
    /// Move 'source' node (plus its children) to become the node at 'target' (erasing the node at 'target').
    template<typename iter> iter move_ontop(iter target, iter source);

    /// Merge with other tree, creating new branches and leaves only if they are not already present.
    void     merge(children_iterator, children_iterator, children_iterator, children_iterator, 
        bool duplicate_leaves=false);
    /// Sort (std::sort only moves values of nodes, this one moves children as well).
    void     sort(children_iterator from, children_iterator to, bool deep=false);
    template<class StrictWeakOrdering>
    void     sort(children_iterator from, children_iterator to, StrictWeakOrdering comp, bool deep=false);
    ...
    tree     subtree(children_iterator from, children_iterator to) const;
    void     subtree(tree&, children_iterator from, children_iterator to) const;
    void     subtree(tree&, iterator i) const;
    /// Exchange the node (plus subtree) with its sibling node (do nothing if no sibling present).
    void     swap(children_iterator it);
    ...
    // 判断ancestor是否是child的祖先节点（父亲/祖父。。。）
    // 即判断child是否在ancestor的子树中
    bool     is_ancestor(const iterator_base& child, const iterator_base& ancestor)
    {
        iterator itr = child;
        if (itr == ancestor)
            return false;
    ...
    children_iterator sibling(const iterator_base& position, unsigned int);  				
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::begin_child(const iterator_base& pos) const
{
    assert(pos.node!=0);
    if(pos.node->first_child==0) {
        return end_child(pos);
    }
    return pos.node->first_child;
}

template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::end_child(const iterator_base& pos) const
{
...
iter tree<T, tree_node_allocator>::append_child(iter position, iter other)
{
    ...
    children_iterator aargh=append_child(position, value_type());
...
iter tree<T, tree_node_allocator>::prepend_child(iter position, iter other)
{
    ...
    children_iterator aargh=prepend_child(position, value_type());
    return replace(aargh, other);
}

template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::append_children(iter position, children_iterator from, children_iterator to)
{
    assert(position.node!=head);
    assert(position.node!=feet);
    assert(position.node);
...
template <class T, class tree_node_allocator>
template <class iter>
iter tree<T, tree_node_allocator>::prepend_children(iter position, children_iterator from, children_iterator to)
{
    assert(position.node!=head);
    assert(position.node!=feet);
    assert(position.node);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::insert(children_iterator position, const T& x)
{
    tree_node* tmp = alloc_.allocate(1,0);
    alloc_.construct(tmp, x);
    //	kp::constructor(&tmp->data, x);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::replace(
    children_iterator orig_begin, 
    children_iterator orig_end, 
    children_iterator new_begin, 
    children_iterator new_end)
{
    if (new_end == new_begin)
        return end_child(root());
...
template <class T, class tree_node_allocator>
template <typename iter>
iter tree<T, tree_node_allocator>::reparent(iter position, children_iterator begin, children_iterator end)
{
...
template <typename iter> iter tree<T, tree_node_allocator>::reparent(iter position, iter from)
{
    ...
    return reparent(position, from.node->first_child, end_child(from));
}
...
{
    assert(position.node!=0);
    children_iterator fr=position, to=position;
    ++to;
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::move_before(children_iterator target, 
    children_iterator source)
{
    tree_node *dst=target.node;
    tree_node *src=source.node;
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::merge(children_iterator to1,   children_iterator to2,
    children_iterator from1, children_iterator from2,
    bool duplicate_leaves)
{
    children_iterator fnd;
    while(from1!=from2) {
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::sort(children_iterator from, children_iterator to, bool deep)
{
    std::less<T> comp;
...
template <class T, class tree_node_allocator>
template <class StrictWeakOrdering>
void tree<T, tree_node_allocator>::sort(children_iterator from, children_iterator to, 
    StrictWeakOrdering comp, bool deep)
{
    ...
    children_iterator it=from, it2=to;
    while(it != to) {
    ...
    if(deep) {	// sort the children of each node too
        children_iterator bcs(*nodes.begin());
        children_iterator ecs(*eit);
        ++ecs;
        while(bcs!=ecs) {
            sort(begin_child(bcs), end_child(bcs), comp, deep);
            ++bcs;
        }
    }
}
...
bool tree<T, tree_node_allocator>::equal_subtree(const iter& one_, const iter& two_, BinaryPredicate fun) const
{
    ...
    return equal(begin_child(one),end_child(one),begin_child(two),fun);
}

template <class T, class tree_node_allocator>
tree<T, tree_node_allocator> tree<T, tree_node_allocator>::subtree(children_iterator from, children_iterator to) const
{
    tree tmp;
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::subtree(tree& tmp, children_iterator from, children_iterator to) const
{
    tmp.set_head(value_type());
    tmp.replace(tmp.begin(), tmp.end(), from, to);
}
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::subtree(tree& t, iterator i) const
{
    if (end() == i)
        return;

    iterator it = t.set_head(*i);
    children_iterator ib_this = begin_child(i);
    children_iterator ie_this = end_child(i);
    if (ib_this != ie_this)
    {
        t.append_child(it, value_type());    // 这个要被替换掉
        t.replace(t.begin_child(it), t.end_child(it), ib_this, ie_this);
    }
}
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::swap(children_iterator it)
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::sibling(const iterator_base& it, unsigned int num)
{
    tree_node *tmp;
    if(it.node->parent==0) {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::iterator_base::begin() const
{
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T, tree_node_allocator>::iterator_base::end() const
{
    children_iterator ret(0);
    ret.parent_=node;
    return ret;
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::pre_order_iterator::pre_order_iterator(const children_iterator& other)
    : iterator_base(other.node)
{
    if(this->node==0) {
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::post_order_iterator::post_order_iterator(const children_iterator& other)
    : iterator_base(other.node)
{
    if(this->node==0) {
        if(other.range_last()!=0)
...
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator& tree<T, tree_node_allocator>::breadth_first_queued_iterator::operator++()
{
    ...
    children_iterator sib=this->begin();
    while(sib!=this->end()) {
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::fixed_depth_iterator::fixed_depth_iterator(const children_iterator& other)
    : iterator_base(other.node), top_node(0)
{
}
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::children_iterator::children_iterator(const iterator_base& other)
    : iterator_base(other.node)
{
    set_parent_();
}
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::children_iterator::children_iterator(const children_iterator& other)
...
template <class T, class tree_node_allocator>
tree<T, tree_node_allocator>::leaf_iterator::leaf_iterator(const children_iterator& other)
    : iterator_base(other.node), top_node(0)
{
    if(this->node==0) {
        if(other.range_last()!=0)

    ...
    int nPixelAutoHide/*=10*/, int nPixelSide/*=4*/ )
{
    ...
    else
    {
        ...
        pAHW->nWndExStyle = GetWindowLong(pAHW->hWnd, GWL_EXSTYLE);
        pAHW->nAlign = nAlign;
    }
}
...
void CAutoHideWnd::HideWnd( LPAHW_DATA pAHW, BOOL bHide )
{
    ...
    {   
        // 显示的时候恢复其原来的窗口属性
//         hInsertAfter = ((pAHW->nWndExStyle) & WS_EX_TOPMOST) ? HWND_TOPMOST : NULL;
//         if (!(pAHW->nWndExStyle & WS_EX_TOPMOST))
//         {
//             SetWindowLong(pAHW->hWnd, GWL_EXSTYLE, pAHW->nWndExStyle);
//         }
    }
    ...
    SetWindowPos(pAHW->hWnd,   hInsertAfter,   xEnd,   yEnd,   0,   0,   SWP_NOSIZE | SWP_NOZORDER);
